...
function Plan() {
  ...
  const planQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("meal_plan")
        .select("id, date, meal_slot, recipe:recipes(id, title)")
        .eq("user_id", user!.id)
        .gte("date", range.from)
        .lte("date", range.to);
  ...
  const recipesQ = useQuery({
    ...
    queryFn: async () => (await supabase.from("recipes").select("id, title, recipe_ingredients(item_name, qty, unit)")).data ?? [],
  });

  const remove = useMutation({
    mutationFn: async (id: string) => { await supabase.from("meal_plan").delete().eq("id", id); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["plan", user!.id, range.from] }),
  });

  const autoPlan = useMutation({
    mutationFn: async () => {
      // Score recipes by how many ingredients use items closest to expiry
      const inv = (inventoryQ.data ?? []).slice().sort((a: any, b: any) => {
      ...
      const recipes = (recipesQ.data ?? []).map((r: any) => {
        ...
        const score = ings.filter((i) => expirySoon.has(normalize(i.item_name))).length;
        return { ...r, score };
      ...
      for (const day of days) {
        for (const slot of slots) {
          ...
          idx++;
          rows.push({ user_id: user!.id, date: format(day, "yyyy-MM-dd"), meal_slot: slot, recipe_id: r.id });
        }
      }
      // Wipe and rewrite week
      ...
      // Sync grocery: missing ingredients
      ...
      for (const r of recipes.slice(0, Math.min(recipes.length, 7))) {
        for (const ing of (r.recipe_ingredients as any[])) {
          ...
          if (!invSet.has(key) && !seen.has(key)) {
            seen.add(key);
            missing.push({ user_id: user!.id, item_name: ing.item_name, qty: ing.qty, unit: ing.unit });
          }
        }
      }
      if (missing.length > 0) await supabase.from("grocery_list").insert(missing);
    },
    onSuccess: () => {
      toast.success("Week planned!");
      qc.invalidateQueries({ queryKey: ["plan", user!.id, range.from] });
      qc.invalidateQueries({ queryKey: ["grocery", user!.id] });
    },
  ...
  return (
    <>
      <ScreenHeader
        ...
        title="Meal plan"
        right={
          <button onClick={() => autoPlan.mutate()} disabled={autoPlan.isPending} className="px-3 py-2 rounded-full bg-primary text-primary-foreground text-xs flex items-center gap-1 disabled:opacity-60">
            <Sparkles className="size-3" /> {autoPlan.isPending ? "Planning…" : "Auto-plan"}
          </button>
        }
      />
                  ...
                  return (
                    <div key={slot} className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex-1">
                        {e ? (
                          <Link to="/recipes/$id" params={{ id: e.recipe.id }} className="text-foreground">{e.recipe.title}</Link>
                        ) : (
                          <Link to="/recipes" className="text-muted-foreground italic">Pick a recipe</Link>
                        )}
                      </div>
                      {e && (
                        <button onClick={() => remove.mutate(e.id)} className="text-muted-foreground p-1"><Trash2 className="size-3.5" /></button>
                      )}
                    </div>
                  );
                })}
